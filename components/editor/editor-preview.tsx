...
export function EditorPreview({ slides, selectedIndex, onSlideChange, onSlideUpdate }: EditorPreviewProps) {
  ...
  const dragRef = useRef<{
    type: "move" | "resize"; targetId: string; targetKind: "field" | "extra"
    sx: number; sy: number; ox: number; oy: number
    moved: boolean; lx: number; ly: number; sw: number; cw: number
  } | null>(null)
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      if (d.type === "move") {
        ...
        if (d.moved) { d.lx = d.ox + dx; d.ly = d.oy + dy; setLiveOffset({ id: d.targetId, x: d.lx, y: d.ly }) }
      } else {
        const pw = Math.max(d.cw * 0.15, d.sw + dx)
        setLW2({ id: d.targetId, w: Math.min(100, (pw / d.cw) * 100) })
      }
    }
    const onUp = () => {
      const d = dragRef.current; if (!d) return
      ...
      if (d.type === "move") {
        if (d.moved) {
          ...
          const same = cur !== null && cur.kind === d.targetKind && ((cur.kind === "field" ? cur.field : cur.id) === d.targetId)
          if (same) {
        ...
        if (lw) {
          if (d.targetKind === "field") onUpdateRef.current?.({ textWidths: { ...(slide.textWidths ?? {}), [d.targetId]: lw.w } })
          else onUpdateRef.current?.({ extraTexts: (slide.extraTexts ?? []).map(e => e.id === d.targetId ? { ...e, width: lw.w } : e) })
          setLW2(null)
        }
      }
  ...
  // helpers
  const getFieldOff = (f: TextFieldKey) => liveOffset?.id === f ? { x: liveOffset.x, y: liveOffset.y } : (slide.textOffsets?.[f] ?? { x: 0, y: 0 })
  const getExtraOff = (id: string) => liveOffset?.id === id ? { x: liveOffset.x, y: liveOffset.y } : ((slide.extraTexts ?? []).find(e => e.id === id)?.offset ?? { x: 0, y: 0 })
  ...
  const startMove = (e: React.MouseEvent, id: string, kind: "field" | "extra", off: { x: number; y: number }) => {
    ...
    dragRef.current = { type: "move", targetId: id, targetKind: kind, sx: e.clientX, sy: e.clientY, ox: off.x, oy: off.y, moved: false, lx: off.x, ly: off.y, sw: 0, cw: 0 }
  }
  const startResize = (e: React.MouseEvent, id: string, kind: "field" | "extra", storedW?: number) => {
    ...
    dragRef.current = { type: "resize", targetId: id, targetKind: kind, sx: e.clientX, sy: e.clientY, ox: 0, oy: 0, moved: false, lx: 0, ly: 0, sw: ((storedW ?? 100) / 100) * cw, cw }
  }
  ...
  // ── toolbar JSX (plain function, not React component) ──
  const renderToolbar = (
    bold: boolean,
    ...
    <div className="absolute -top-11 left-0 flex items-center gap-1 bg-black/85 backdrop-blur-sm rounded-xl px-2 py-1.5 z-50 shadow-xl"
      ...
      <div className="w-px h-4 bg-white/20 mx-0.5" />
      <span className="text-white/40 text-[10px] whitespace-nowrap">{editing ? "Enter·저장 Esc·취소" : "클릭: 편집"}</span>
    </div>
  )

  // ── field wrapper (plain function) ──
  const wrap = (
    ...
    return (
      ...
      >
        <div className={cn("absolute -inset-1.5 rounded pointer-events-none border-2 transition-opacity",
        ...
        {(isSelEl || isEditEl) && (
          <div className="absolute -right-2 top-0 bottom-0 w-4 flex items-center justify-center cursor-ew-resize z-40"
            onMouseDown={e => startResize(e, targetId, targetKind, storedW)} onClick={e => e.stopPropagation()}>
            <div className={cn("w-1.5 h-10 max-h-full rounded-full", isRes ? "bg-blue-300" : "bg-blue-500 hover:bg-blue-300")} />
          </div>
        )}
        {children}
      </div>
  ...
  const renderTextarea = (onBlur: () => void, noNewline?: boolean) => (
    <textarea autoFocus
      ...
      style={{ color: "inherit", fontFamily: "inherit", fontSize: "inherit", fontWeight: "inherit", lineHeight: "inherit", textAlign: "inherit" as React.CSSProperties["textAlign"], minWidth: "8ch", padding: 0, border: "none" }}
      value={editingValue}
  ...
  const renderField = (field: TextFieldKey, value: string, staticEl: React.ReactNode) => {
    ...
    return wrap(field, "field", isSelEl, isEditEl, off, getW(field, slide.textWidths?.[field]), bold, toolbar,
      isEditEl ? renderTextarea(commitField, field !== "content") : staticEl, slide.textWidths?.[field])
  }
  ...
  const renderExtra = (et: ExtraText) => {
    ...
    return wrap(et.id, "extra", isSelEl, isEditEl, off, getW(et.id, et.width), et.bold, toolbar,
      isEditEl
    ...
    <div className="flex flex-col items-center h-full py-6 px-4 gap-4"
      ...
      <div className="flex items-center gap-4 w-full max-w-sm">
        <div className="flex items-center gap-1 p-1 rounded-lg bg-white/5 border border-white/10">
          <button onClick={() => setAspectMode("square")} className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-all", aspectMode === "square" ? "bg-primary text-primary-foreground shadow" : "text-white/50 hover:text-white/80")}><Square className="w-3 h-3" />1:1</button>
          <button onClick={() => setAspectMode("story")} className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-all", aspectMode === "story" ? "bg-primary text-primary-foreground shadow" : "text-white/50 hover:text-white/80")}><Smartphone className="w-3 h-3" />9:16</button>
        </div>
        {clip && (
          <button onClick={handlePaste} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-500/20 border border-blue-400/40 text-blue-300 hover:bg-blue-500/30 text-xs font-medium transition-all">
            <Clipboard className="w-3.5 h-3.5" />붙여넣기</button>
        )}
        <div className="flex items-center gap-2 ml-auto">
          <button onClick={goToPrev} disabled={selectedIndex === 0} className="w-7 h-7 rounded-lg flex items-center justify-center border border-white/10 text-white/50 hover:text-white hover:border-white/30 disabled:opacity-30 transition-all"><ChevronLeft className="w-4 h-4" /></button>
          <span className="text-xs text-white/40 tabular-nums w-10 text-center">{selectedIndex + 1} / {slides.length}</span>
          <button onClick={goToNext} disabled={selectedIndex === slides.length - 1} className="w-7 h-7 rounded-lg flex items-center justify-center border border-white/10 text-white/50 hover:text-white hover:border-white/30 disabled:opacity-30 transition-all"><ChevronRight className="w-4 h-4" /></button>
        </div>
      </div>

      {/* 카드 미리보기 */}
      <div className="flex-1 flex items-center justify-center w-full">
        <div ref={cardRef} className={cn("relative w-full max-w-sm shadow-2xl shadow-black/50 overflow-visible", aspectMode === "square" ? "aspect-square" : "aspect-[9/16]")} style={{ borderRadius: 12 }}>

          {/* 배경 — pointer-events-none 으로 클릭 차단 안 함 */}
          ...
          {slide.productImageUrl && (
            <div className={cn("absolute left-1/2 -translate-x-1/2 z-10 pointer-events-none",
              (slide.productImagePosition ?? "top") === "top" ? "top-6" : (slide.productImagePosition ?? "top") === "center" ? "top-1/2 -translate-y-1/2" : "bottom-6")}
          ...
          <div className="absolute inset-0 flex flex-col p-6 md:p-8"
            ...
            <div className={cn("flex flex-col gap-3", alignClass)}>
              {renderField("subtitle", slide.subtitle ?? "", <p className="text-sm font-medium opacity-80 whitespace-pre-line" style={{ color: slide.bgStyle.titleColor }}>{slide.subtitle}</p>)}
              {renderField("title", slide.title, <h2 className={cn("font-bold leading-tight whitespace-pre-line", titleClass)} style={{ color: slide.bgStyle.titleColor }}>{slide.title || "제목을 입력하세요"}</h2>)}
              {renderField("content", slide.content, <p className={cn("leading-relaxed whitespace-pre-line", contentClass)} style={{ color: slide.bgStyle.textColor }}>{slide.content || "내용을 입력하세요"}</p>)}
          ...
          <div className="absolute inset-0 p-6" style={{ fontFamily: fontCss }}>
            <div className="relative w-full h-full">
              {(slide.extraTexts ?? []).map(et => <div key={et.id} className="absolute top-0 left-0">{renderExtra(et)}</div>)}
            </div>
          </div>

          <div className="absolute top-3 right-3 z-10">
            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: "rgba(0,0,0,0.3)", color: "rgba(255,255,255,0.7)", backdropFilter: "blur(4px)" }}>{selectedIndex + 1}/{slides.length}</span>
          </div>
        </div>
      </div>

      <div className="flex items-center gap-1.5" onClick={e => e.stopPropagation()}>
        {slides.map((_, i) => <button key={i} onClick={() => onSlideChange(i)} className={cn("h-1.5 rounded-full transition-all", i === selectedIndex ? "w-6 bg-primary" : "w-1.5 bg-white/25 hover:bg-white/50")} />)}
      </div>
    </div>
  )
}
